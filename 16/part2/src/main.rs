...
fn main() {
    ...
    let max_energised = (0..grid_width)
        .map(|col| ((col, 0), Direction::Down))
        .chain((0..grid_height).map(|row| ((grid_width - 1, row), Direction::Left)))
        .chain((0..grid_width).map(|col| ((col, grid_height - 1), Direction::Up)))
        .chain((0..grid_height).map(|row| ((0, row), Direction::Right)))
        .map(|(coords, direction)| {
            let mut energised_tiles = HashMap::new();
            follow_beam(&grid, &mut energised_tiles, coords, direction);
            energised_tiles.len()
        })
        .max()
        .expect("grid was empty");
    println!("A maximum of {max_energised} tiles can be energised!");
}
